 ...
 */
import { Context, EntityConfiguration } from '../../types/core';
...
export class WorkflowManager {
  ...
  async executeWorkflows(
    ...
    const matchingWorkflows = entityConfig.workflows.filter(workflow => {
      ...
      if (!workflow.workflow_is_active) {
        return false;
      }

      // Check if workflow trigger matches the event
      return workflow.workflow_trigger_event === event;
    });
  ...
  private async executeWorkflow(
    ...
    // Check conditions
    if (workflow.workflow_conditions && !this.evaluateWorkflowConditions(workflow.workflow_conditions, oldData, newData, context)) {
      return; // Conditions not met, skip workflow
    }
    
    // Execute actions
    if (workflow.workflow_actions && Array.isArray(workflow.workflow_actions)) {
      await this.executeWorkflowActions(workflow.workflow_actions, event, oldData, newData, context);
    }
  }
