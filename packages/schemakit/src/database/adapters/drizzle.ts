...
export class DrizzleAdapter extends DatabaseAdapter {
  ...
  async insert(table: string, data: Record<string, any>): Promise<any> {
    try {
      ...
      if (this.dbType === 'postgres') {
        // Use query path to fetch returning rows
        const rows = await this.query(`${sql} RETURNING *`, params);
        return rows[0] || { ...data };
      }
      const res = await this.execute(sql, params);
      return { id: res.lastInsertId, ...data };
    } catch (error) {
      throw new DatabaseError('execute', { cause: error as any, context: { sql: `INSERT INTO ${table}`, params: data } });
    }
  }

  async update(table: string, idField: string, id: string | number, data: Record<string, any>): Promise<any> {
    try {
      const { sql, params } = SqlBuilder.buildUpdate(table, id, data, this.dbType, idField);
      const res = await this.execute(sql, params);
      if (res.changes === 0) throw new Error(`No record found with id: ${id}`);
      return { id, ...data };
    } catch (error) {
      throw new DatabaseError('execute', { cause: error as any, context: { sql: `UPDATE ${table}`, params: { idField, id, ...data } } });
    }
  }

  async delete(table: string, idField: string, id: string | number): Promise<void> {
    try {
      const { sql, params } = SqlBuilder.buildDelete(table, id, this.dbType, idField);
      const res = await this.execute(sql, params);
      if (res.changes === 0) throw new Error(`No record found with id: ${id}`);
    } catch (error) {
      throw new DatabaseError('execute', { cause: error as any, context: { sql: `DELETE FROM ${table}`, params: { idField, id } } });
    }
  }
