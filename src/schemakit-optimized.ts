/**
 * SchemaKit - Optimized Version
 * Uses all simplified components for maximum performance and maintainability
 */
import { DatabaseAdapter } from './database/adapter';
import { EntityConfiguration, Context } from './types-simplified';
...
import { QueryManager } from './core/query-manager-simplified';
...
/**
 * SchemaKit - Optimized main class
 * Focus: Core entity management with minimal complexity
 */
export class SchemaKit {
  ...
  static async shutdown(instanceName?: string): Promise<void> {
    return InstanceManager.shutdown(instanceName);
  }

  static async shutdownAll(): Promise<void> {
    return InstanceManager.shutdownAll();
  }
  ...
  private createDatabaseAdapter(type: string, config: Record<string, any>): DatabaseAdapter {
    switch (type.toLowerCase()) {
      ...
      case 'inmemory':
      default:
        const { InMemoryAdapter } = require('./database/adapters/inmemory-simplified');
