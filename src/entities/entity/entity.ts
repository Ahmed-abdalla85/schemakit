...
export class Entity {
  ...
  static create(entityName: string, tenantId: string, databaseManager: DatabaseManager): Entity {
    const cacheKey = `${tenantId}:${entityName}`;
    if (Entity.cache.has(cacheKey)) return Entity.cache.get(cacheKey)!;
    const entity = new Entity(entityName, tenantId, databaseManager);
    Entity.cache.set(cacheKey, entity);
    return entity;
  }

  private constructor(entityName: string, tenantId: string, databaseManager: DatabaseManager) {
  ...
  async initialize(context: Context = {}): Promise<void> {
    ...
    try {
      ...
      const [fields, permissions, workflows, rls, views] = await Promise.all([
        this.loadFields(this.entityDefinition.id),
        this.loadPermissions(this.entityDefinition.id, contextWithTenant),
        this.loadWorkflows(this.entityDefinition.id),
        this.loadRLS(this.entityDefinition.id, contextWithTenant),
        this.loadViews(this.entityDefinition.id)
      ]);
      this.fields = fields;
  ...
  async create(data: Record<string, any>, context: Context = {}): Promise<Record<string, any>> {
    ...
    if (result.changes === 0) {
      throw new SchemaKitError(`Failed to create ${this.entityName} record`);
    }
  ...
  async read(filters: Record<string, any> = {}, context: Context = {}): Promise<Record<string, any>[]> {
    ...
    let query = this.databaseManager.db(this.getTableName());
  ...
  async update(id: string | number, data: Record<string, any>, context: Context = {}): Promise<Record<string, any>> {
    ...
    let query = this.databaseManager.db(this.getTableName()).where('id', '=', id);
    ...
    if (result.changes === 0) {
      throw new SchemaKitError(`Update failed or permission denied: ${this.entityName} with ID ${id}`);
    }
  ...
  private getTableName(): string {
    return this.entityDefinition?.table_name || this.entityName;
  }

  private async loadEntityDefinition(): Promise<EntityDefinition | null> {
    const results = await this.databaseManager.query<EntityDefinition>(
      'SELECT * FROM system_entities WHERE name = ? AND is_active = 1',
      [this.entityName]
    );
  ...
  private async loadFields(entityId: string): Promise<FieldDefinition[]> {
    const fields = await this.databaseManager.query<FieldDefinition>(
      'SELECT * FROM system_fields WHERE entity_id = ? AND is_active = 1 ORDER BY order_index ASC',
      [entityId]
    );
  ...
  private async loadPermissions(entityId: string, context: Context): Promise<PermissionDefinition[]> {
    ...
    const permissions = await this.databaseManager.query<PermissionDefinition>(
      `SELECT * FROM system_permissions WHERE entity_id = ? AND role IN (${placeholders}) AND is_active = 1`,
      [entityId, ...userRoles]
    );
  ...
  private async loadWorkflows(entityId: string): Promise<WorkflowDefinition[]> {
    const workflows = await this.databaseManager.query<WorkflowDefinition>(
      'SELECT * FROM system_workflows WHERE entity_id = ? AND is_active = 1 ORDER BY order_index ASC',
      [entityId]
    );
  ...
  private async loadRLS(entityId: string, context: Context): Promise<RLSDefinition[]> {
    ...
    const rlsRules = await this.databaseManager.query<RLSDefinition>(
      `SELECT * FROM system_rls WHERE entity_id = ? AND role IN (${placeholders}) AND is_active = 1`,
      [entityId, ...userRoles]
    );

    return rlsRules.map(rule => ({
      ...rule,
  ...
  private async ensureTable(): Promise<void> {
    ...
    if (!exists) {
      const columns = this.fields.map(field => ({
        name: field.name,
        type: this.mapFieldTypeToSQL(field.type),
