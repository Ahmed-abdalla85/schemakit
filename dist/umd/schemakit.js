(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "./database/adapter", "./errors"], factory);
    }
    ...
    class SchemaKit {
        ...
        async findByView(entityName, viewName, params = {}, context = {}) {
            try {
                ...
                if (!view) {
                    throw new errors_1.SchemaKitError(`View '${viewName}' not found for entity '${entityName}'`);
                }
                // Ensure entity table exists
                await this.ensureEntityTable(entityConfig);
                // Apply RLS (Row Level Security) conditions
        ...
        async update(entityName, id, data, context = {}) {
            try {
                ...
                if (!currentData) {
                    throw new errors_1.SchemaKitError(`Entity ${entityName} with id ${id} not found`);
                }
                // Validate data against entity schema
                ...
                if (!updatedData) {
                    throw new errors_1.SchemaKitError(`Failed to retrieve updated entity ${entityName} with id ${id}`);
                }
        ...
        async checkPermission(entityName, action, context = {}) {
            try {
                ...
                // Check if any permission allows the action
                return entityConfig.permissions.some(p => p.action === action &&
                    p.is_allowed &&
                    userRoles.includes(p.role));
            }
            catch (error) {
